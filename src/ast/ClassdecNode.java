...
public class ClassdecNode implements Node {
	...
	public Node typeCheck() {
		...
		for (String i : id) {
			...
			// Controllo se ш dichiarato un campo con le stesso id in qualche superclasse:
	...
	private boolean searchInSuper(String id, String superc, HashMap<String, STentry> hm2) {
		// Se superc ш null, il campo non era una classe oppure abbiamo giра controllato
	...
	// Data un VardecContext e un id di una classe, controlla se c'ш un campo con lo
	...
	// Se non c'ш nessun campo con lo stesso id, restituisce true
		...
		for (VardecContext f1 : superEntry.getListContext()) {
			...
			if ((d.ID().getText().equals(f1.ID().getText()))) {
				...
				if (d.type().getText().equals("int") || d.type().getText().equals("bool")) {
					// Se il tipo del campo della sottoclasse ш int o bool, controllo che quello
					...
					// Se il tipo non ш ne int ne bool (e quindi ш una classe), controllo che la
